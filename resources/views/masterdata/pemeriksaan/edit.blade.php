	...
	<div class="col-md-12">
		<div class="panel panel-primary" data-collapsed="0">
			<div class="panel-body">
				<form action="{{route('pemeriksaan.update', $data_pemeriksaan->id_dpemeriksaan)}}" role="form" class="form-horizontal form-groups-bordered" method="post">
					{{ csrf_field() }}
								...
								<div class="col-sm-5">
									<select name="id_kategori" class="select2" required data-placeholder="Pilih Kategori...">
											...
											@foreach ($kategori as $kategori)
												<option value="{{$kategori->id_kategori}}" @if($data_pemeriksaan->id_kategori == $kategori->id_kategori) selected @endif>{{$kategori->nama_kategori}}</option>
											@endforeach
											</optgroup>
									</select>
								</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" required="required" class="form-control" id="nama_pemeriksaan" name="nama_pemeriksaan" placeholder="nama pemeriksaan" value="{{$data_pemeriksaan->nama_pemeriksaan}}"/>
						</div>
					</div>
